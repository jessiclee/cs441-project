...
def handle_ip_packet(data, macsrc, local):
    ...
    try:
        if local == 1:
            ...
            print( f"Sending packet through R1 interface to N1 with headers: ipsrc {ipsrc}, ipdst {ipdst}, macsrc {MAC1}, macdst {dstmac}, {pack}" )
            n1.sendall(pack)
        elif local == 2:
            dstmac = R2_IDs[ipdst]
            pack = create_ether_frame(data, MAC2, dstmac, len)
            print( f"Sending through R2 interface to N2/3 with headers: ipsrc {ipsrc}, ipdst {ipdst}, macsrc {MAC1}, macdst {dstmac}, {pack}")
            n2.sendall(pack)
            n3.sendall(pack)
    except KeyError:
